<?php
    ...
    if(isset($_POST['change']) && isset($_POST['active'])!="") {
        ...
        $sql = "UPDATE `images_portfolio` SET `active` = '$active' WHERE `id` = '$id'";
        $mysqli -> query($sql);
    }
    
    if(isset($_POST['delete']) && is_array($_POST['delete'])) {
        foreach($_POST['delete'] as $id => $useless){
            $sql = "DELETE FROM `images_portfolio` WHERE `images_portfolio`.`id` = '$id'";
            $mysqli -> query($sql);
            ?> 
                <script>
                    alert("Вы удалили фото!");
                    document.location.replace("images_portfolio.php");
                </script> 
            <?php
        }
    }
    ...
    if(isset($_POST['add'])) {
        $name = $_POST['name'];
        $uploaddir = './img/content/screen_fourth/';
        ...
        $sql = "INSERT INTO `images_portfolio` (`name/alt`, `way`) VALUES ('$name', '$way')";
        $mysqli -> query($sql);
        ?> 
            <script>
                alert("Вы добавили фото!");
                document.location.replace("images_portfolio.php");
            </script> 
        <?php
    }
...
<html lang="ru">
    ...
    <body class="center_admin">
        ...
        <?php $result = $mysqli-> query("SELECT * FROM `images_portfolio`");
            foreach($result as $rows) {?>
                ...
                <div class="screen_admin">
                    <p class="text_admin left_block">
                        ...
                        <span class="second_table_swiper"><img class="admin_image" src="./img/content/screen_fourth/<?php echo $rows['way']?>"></span> <span class="none1150"><br class="none1150"></span>
