<?php
    ...
    if(isset($_POST['change']) && isset($_POST['active'])!="") {
        ...
        $sql = "UPDATE `swiper_about_us` SET `active` = '$active' WHERE `id` = '$id'";
        $mysqli -> query($sql);
    }
    
    if(isset($_POST['delete']) && is_array($_POST['delete'])) {
        foreach($_POST['delete'] as $id => $useless){
            $sql = "DELETE FROM `swiper_about_us` WHERE `swiper_about_us`.`id` = '$id'";
            $mysqli -> query($sql);
            ?> 
                <script>
                    alert("Вы удалили слайд!");
                document.location.replace("swiper_about_us.php");
                </script> 
            <?php
        }
    }
    ...
    if(isset($_POST['add'])) {
        ...
        $sql = "INSERT INTO `swiper_about_us` (`name/alt`, `way`) VALUES ('$name', '$way')";
        $mysqli -> query($sql);
        ?> 
            <script>
...
<html lang="ru">
    <head>
        ...
        <title>Фото слайдера "О нас"</title>
    </head>
    ...
    <body class="center_admin">
        ...
        <?php $result = $mysqli-> query("SELECT * FROM `swiper_about_us`");
            foreach($result as $rows) {?>
                ...
                <div class="screen_admin">
                    <p class="text_admin left_block">
                        ...
                        <span class="second_table_swiper"><img class="admin_image" src="./img/swiper_second/<?php echo $rows['way']?>"></span> <span class="none1150"><br class="none1150"></span>
